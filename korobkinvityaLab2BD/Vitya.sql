...
WHILE @step <= 500
	BEGIN
		...
		INSERT INTO Clients (FIO, Number, Pasport) VALUES (@FIO, 1 + RAND() * 1000000, @Pass)
		SET @step = @step + 1;
	END;
...
WHILE @step <= 1000
	BEGIN
		...
		INSERT INTO Employees (FIO, Position, DateOfWorkStart) VALUES (@FIO, @Pos, DATEADD(d, RAND() * 100, GETDATE()))
		SET @step = @step + 1;
	END;
...
WHILE @step <= 1500
	BEGIN
		...
		INSERT INTO Genres([Name], [Description]) VALUES (@Name, @Descr);
		SET @step = @step + 1;
	END;
...
WHILE @step <= 21500
	BEGIN
		...
		INSERT INTO Discs([Name], DateOfCreation ,Creater, Country, MainActor, DateOfRecord, Genre, TypeOfDisc, Price) VALUES (@Name, DATEADD(d, RAND() * 100, GETDATE()), @Creator, @Country, @MainActor, DATEADD(d, RAND() * 100, GETDATE()), @Genre, @type, 1 + RAND() * 1000);
		SET @step = @step + 1;
	END;
...
WHILE @step <= 41500
	BEGIN
		...
		INSERT INTO RentalRecords (ClientId, DateOfRent ,DateOfReturn, PaymentCheck, ReturnCheck, DiscId, EmployeeId) VALUES (@Client, DATEADD(d, RAND() * 100, GETDATE()), DATEADD(d, RAND() * 100, GETDATE()), ROUND(RAND(), 1), ROUND(RAND(), 1), @Disc, @Empl);
		SET @step = @step + 1;
	END;